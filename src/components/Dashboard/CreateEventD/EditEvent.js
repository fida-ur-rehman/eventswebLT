...
function EditEvent(props) {
    ...
    let details = props.location.state.item
    ...
    const getCurr = async ()=>{
      ...
      const res = await axios.get(`https://api.currencyapi.com/v3/latest?apikey=e342da80-89cc-11ec-b105-457f4145383a&base_currency=${props.user.userInfo.curr}`)
      console.log(res)
    ...
    React.useEffect(()=>{
      ...
      setValue("eventAddress",details.eventAddress)

    }
        setValue("eventName",details.name)
    ...
    const onSubmit = (data,e)=>{
       // let correctedStart=date.parse(start,'DD/MM/YYYY h:mm A')
        //let correctedEnd=date.parse(end,'DD/MM/YYYY h:mm A') location:props.location.state.eventLocation, 
      console.log(data);
      setOpen(true)
      axios.post(`${process.env.REACT_APP_DEVELOPMENT}/api/event/update-event`,
        {eventId:details._id,data:[
            {"propName" : "name", "value": data.eventName},
            {"propName" : "mobileNo", "value": phone},
            ...
            {"propName" : "location", "value": props.location.state.eventLocation},
            {"propName" : "eventAddress", "value": data.eventAddress}
    ...
    return (
        ...
        <h1 className="heading1">Edit Event</h1>
